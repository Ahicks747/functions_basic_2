# --- Question 1 ---
# Countdown - Create a function that accepts a number as an input. Return a new list that counts down by one, from the number (as the 0th element) down to 0 (as the last element).
# Example: countdown(5) should return [5,4,3,2,1,0]

#  Option 1)
def countdown (num1):   
    return list(range(num1,-1,-1))
print (countdown(5))

# #  Option 2)
def countdown (num1):
    dec_list = []
    for num1 in range (num1, -1, -1):
        dec_list.append(num1)
    return dec_list
countdown(5)

# --- Question 2 ---
# Print and Return - Create a function that will receive a list with two numbers. Print the first value and return the second.
# Example: print_and_return([1,2]) should print 1 and return 2

def print_and_return (list):
    print (list[0])
    return (list[1])
print_and_return([3,6])
# Issues I had was trying to make a ([a,b]) which was giving error. Seeing that with a list, you just name it list and identify
# each element within a list you what to work with. Since we are only doing 2, it would be index 0 and 1.

# --- Question 3 ---
# First Plus Length - Create a function that accepts a list and returns the sum of the first value in the list plus the list's length.
# Example: first_plus_length([1,2,3,4,5]) should return 6 (first value: 1 + length: 5)

def first_plus_length(list):
    sum = list[0] + len(list)
    # print(sum)
    return sum
first_plus_length([1,2,3,4,5])

# --- Question 4 ---
# Values Greater than Second - Write a function that accepts a list and creates a new list containing only the values from the original list that are greater than its 2nd value. 
# Print how many values this is and then return the new list. If the list has less than 2 elements, have the function return False
# Example: values_greater_than_second([5,2,3,2,1,4]) should print 3 and return [5,3,4]
# Example: values_greater_than_second([3]) should return False

def values_greater_than_second(list):
    newList= []
    if len(list) < 2:
        return False
    for i in list:
        if i > list[1]:
            newList.append(i)
    print(len(newList))
    return newList
values_greater_than_second([5,2,3,2,1,4])
values_greater_than_second([3])
# Found this to be a littler easier on the eye than using len(list[]) and what not. Still trying to get used to not having put in a incremental counter and it still working or at least seeing 'i in list' as a counter.


# --- Question 5 ---
# This Length, That Value - Write a function that accepts two integers as parameters: size and value. The function should create and return a list whose length is equal to the given size, 
# and whose values are all the given value.
# Example: length_and_value(4,7) should return [7,7,7,7]
# Example: length_and_value(6,2) should return [2,2,2,2,2,2]

def length_and_value(size,value):
    superList = []
    while len(superList) <= size:
        superList.append(value)
    return superList
print(length_and_value(4,7))
print(length_and_value(6,2))